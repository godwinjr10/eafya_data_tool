...
const OutpatientReport = ({ data, reportPeriod }) => {
  const MyDocument = () => (
    <Document>
      <Page size="A4" orientation="landscape" style={styles.page}>
        <View style={styles.header}>
          ...
          <View style={styles.headerText}>
            ...
            <Text style={styles.subtitle}>HMIS Outpatient Report</Text>
        ...
        <View style={styles.table}>
          ...
          {data.map((item, index) => (
            <View key={index} style={styles.tableRow}>
              ...
              <View style={[styles.tableCell, styles.textCell]}>
                <Text>{item.visit_type_name || '-'}</Text>
              </View>
              <View style={[styles.tableCell, styles.textCell]}>
  ...
  const downloadPDF = async () => {
    try {
      ...
      const date = new Date();
      const filename = `HMIS_Outpatient_Report_${date.getFullYear()}_${date.getMonth() + 1}_${date.getDate()}.pdf`;
