...
const ProcedureModal = ({ isOpen, onClose, onSubmit, hmisCode, hmisName }) => {
  ...
  const fetchItems = async (search = '') => {
    ...
    try {
      ...
      const res = await API.get(`/mapping/procedures/items?${params.toString()}`)
      setItems(res.data || [])
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        hmis_code: hmisCode,
        hmis_name: hmisName,
        procedures: selectedItems.map(it => ({ procedure_id: it.id, procedure_name: it.name }))
      }
      await API.post('/mapping/procedures', payload)
      onSubmit()
      handleClose()
    } catch (err) {
