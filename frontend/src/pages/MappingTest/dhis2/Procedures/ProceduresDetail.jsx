...
const ProceduresDetail = () => {
  ...
  const fetchMappingDetail = useCallback(async () => {
    ...
    try {
      const res = await API.get(`/mapping/procedures/${hmisCode}`);
  ...
  const handleDelete = async (id) => {
    try {
      await API.delete(`/mapping/procedures/${id}`);
  ...
  return (
    <div className="container-fluid py-3">
          ...
          <div className="table-responsive">
            <table className="table table-hover" style={{ backgroundColor: '#ffffff' }}>
              <thead style={{ backgroundColor: '#ffffff' }}>
                <tr>
                  <th className="border-0 py-3 px-3" style={{ color: '#2c3e50', fontWeight: '600' }}>Procedure ID</th>
                  <th className="border-0 py-3 px-3" style={{ color: '#2c3e50', fontWeight: '600' }}>Procedure Name</th>
                  ...
                  mappingData.map((row, index) => (
                    ...
                    >
                      <td className="py-3 px-3 border-0" style={{ color: '#495057' }}>
                        <span className="badge bg-light text-dark" style={{ fontSize: '0.8rem' }}>{row.procedure_id}</span>
                      </td>
                      <td className="py-3 px-3 border-0" style={{ color: '#495057' }}>{row.procedure_name}</td>
      ...
      <ProceduresModal isOpen={isModalOpen} onClose={handleModalClose} onSubmit={handleModalSubmit} hmisCode={hmisCode} hmisName={hmisName} />
    </div>
  )
}
