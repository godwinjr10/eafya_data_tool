...
const ImagingModal = ({ isOpen, onClose, onSubmit, hmisCode, hmisName }) => {
  ...
  const fetchItems = async (search = '') => {
    ...
    try {
      ...
      const res = await API.get(`/mapping/imaging/items?${params.toString()}`)
      setItems(res.data || [])
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        hmis_code: hmisCode,
        hmis_name: hmisName,
        imaging: selectedItems.map(it => ({ imaging_id: it.id, imaging_name: it.name }))
      }
      await API.post('/mapping/imaging', payload)
      onSubmit()
      handleClose()
    } catch (err) {
