...
const ImagingDetail = () => {
  ...
  const fetchMappingDetail = useCallback(async () => {
    ...
    try {
      const res = await API.get(`/mapping/imaging/${hmisCode}`);
  ...
  const handleDelete = async (id) => {
    try {
      await API.delete(`/mapping/imaging/${id}`);
  ...
  return (
    <div className="container-fluid py-3">
          ...
          <div className="table-responsive">
            <table className="table table-hover" style={{ backgroundColor: '#ffffff' }}>
              <thead style={{ backgroundColor: '#ffffff' }}>
                <tr>
                  <th className="border-0 py-3 px-3" style={{ color: '#2c3e50', fontWeight: '600' }}>Imaging ID</th>
                  <th className="border-0 py-3 px-3" style={{ color: '#2c3e50', fontWeight: '600' }}>Imaging Name</th>
                  ...
                  mappingData.map((row, index) => (
                    ...
                    >
                      <td className="py-3 px-3 border-0" style={{ color: '#495057' }}>
                        <span className="badge bg-light text-dark" style={{ fontSize: '0.8rem' }}>{row.imaging_id}</span>
                      </td>
                      <td className="py-3 px-3 border-0" style={{ color: '#495057' }}>{row.imaging_name}</td>
      ...
      <ImagingModal isOpen={isModalOpen} onClose={handleModalClose} onSubmit={handleModalSubmit} hmisCode={hmisCode} hmisName={hmisName} />
    </div>
  )
}
