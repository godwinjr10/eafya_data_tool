...
const Conditions = () => {
  ...
  const fetchMappings = useCallback(async (pageNum = 1, search = '', section = '') => {
    try {
      ...
      const response = await API.get('/mapping/conditions', { params })
  ...
  const fetchSections = useCallback(async () => {
    try {
      const response = await API.get('/mapping/conditions/sections')
      if (response.data) {
            ...
            {mappings.length === 0 ? (
              ...
              mappings.map((item, index) => (
                <tr 
                  ...
                  onClick={() => history.push(`/mapping/${item.hmis_code}`, {
