...
const Vaccines = () => {
  ...
  const fetchMappings = async (pageNum = 1, search = '') => {
    try {
      ...
      const res = await API.get('/mapping/vaccines', { params })
      if (res.data) {
  ...
  if (loading && mappings.length === 0) {
    return (
      <div className="text-center py-5">
        ...
        <p className="mt-3 text-muted">Loading Vaccines mappings...</p>
      </div>
    )
  }
      ...
      <div className="row mb-3">
        ...
        <div className="col-md-4 text-end">
          <small className="text-muted d-block">
            Showing {mappings.length} of {total} vaccines
          </small>
        </div>
      </div>
            ...
            {mappings.length === 0 ? (
              <tr>
                <td colSpan={3} className="text-center py-5">
                  ...
                  <p className="text-muted mb-0">No Vaccines found</p>
                </td>
              </tr>
            ) : (
              mappings.map((item, idx) => (
                <tr
                  ...
                  role="button"
                  onClick={() => history.push(`/mapping/vaccines/${item.hmis_code}`, {
