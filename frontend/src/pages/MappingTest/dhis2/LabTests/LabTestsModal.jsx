...
const LabTestsModal = ({ isOpen, onClose, onSubmit, hmisCode, hmisName }) => {
  ...
  const fetchItems = async (search = '') => {
    ...
    try {
      ...
      const res = await API.get(`/mapping/labtests/items?${params.toString()}`)
      setItems(res.data || [])
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        hmis_code: hmisCode,
        hmis_name: hmisName,
        labtests: selectedItems.map(it => ({ labtest_id: it.id, labtest_name: it.name }))
      }
      await API.post('/mapping/labtests', payload)
      onSubmit()
      handleClose()
    } catch (err) {
