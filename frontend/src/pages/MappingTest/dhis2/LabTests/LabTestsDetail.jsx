...
const LabTestsDetail = () => {
  ...
  const fetchMappingDetail = useCallback(async () => {
    ...
    try {
      const res = await API.get(`/mapping/labtests/${hmisCode}`);
      setMappingData(res.data || []);
    } catch (error) {
      console.error("Error fetching labtest mapping detail:", error);
      setMappingData([]);
    }
  }, [hmisCode]);
  ...
  const handleDelete = async (id) => {
    try {
      await API.delete(`/mapping/labtests/${id}`);
  ...
  const handleAdd = () => {
    if (hmisCode) setIsModalOpen(true)
  };
  ...
  return (
    <div className="container-fluid py-3">
          ...
          <div className="row align-items-center">
            <div className="col-lg-8">
              ...
              <div className="header-content">
                <h1 className="page-title-modern">{hmisName}</h1>
                <div className="page-meta-simple">
                  <span className="meta-code"><i className="fas fa-code me-2"></i>{hmisCode}</span>
                </div>
              </div>
            </div>
            <div className="col-lg-4 text-end">
              <button className="btn btn-primary btn-modern" onClick={handleAdd}>
                <i className="fas fa-plus me-2"></i>Add New Mapping
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="table-responsive">
            <table className="table table-hover" style={{ backgroundColor: '#ffffff' }}>
              <thead style={{ backgroundColor: '#ffffff' }}>
                <tr>
                  <th className="border-0 py-3 px-3" style={{ color: '#2c3e50', fontWeight: '600' }}>LabTest ID</th>
                  <th className="border-0 py-3 px-3" style={{ color: '#2c3e50', fontWeight: '600' }}>LabTest Name</th>
                  ...
                  mappingData.map((row, index) => (
                    ...
                    >
                      <td className="py-3 px-3 border-0" style={{ color: '#495057' }}>
                        <span className="badge bg-light text-dark" style={{ fontSize: '0.8rem' }}>{row.labtest_id}</span>
                      </td>
                      <td className="py-3 px-3 border-0" style={{ color: '#495057' }}>{row.labtest_name}</td>
      ...
      <LabTestsModal isOpen={isModalOpen} onClose={handleModalClose} onSubmit={handleModalSubmit} hmisCode={hmisCode} hmisName={hmisName} />
    </div>
  )
}

export default LabTestsDetail;
