...
const Facility = () => {
  ...
  const [dhis2Uri, setDhis2Uri] = useState("");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const facilityData = {
        ...
        dhis2_uri: dhis2Uri || null,
        dhis2_username: dhis2Username || null,
        dhis2_password: dhis2Password || null,
      };
      ...
      showAlert(
        error.response?.data?.message || "Error saving facility",
        "danger"
      );
    }
  };
  ...
  const handleEdit = (facility) => {
    ...
    setDhis2Uri(facility.dhis2_uri || "");
    setDhis2Username(facility.dhis2_username || "");
    setDhis2Password(facility.dhis2_password || "");
    setShowModal(true);
  };
        ...
        <tbody>
          {facilities.map((facility) => (
            <tr key={facility.id}>
              ...
              <td>{facility.dhis2_uri || "-"}</td>
              <td>{facility.dhis2_username || "-"}</td>
              <td>
                <Button
                  variant="primary"
                  size="sm"
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)}>
        <Form onSubmit={handleSubmit}>
          ...
          <Modal.Body>
            ...
            <Form.Group className="mb-3">
              <Form.Label>DHIS2 URI</Form.Label>
              <Form.Control
                type="url"
                placeholder="Enter DHIS2 URI (optional)"
                value={dhis2Uri}
                onChange={(e) => setDhis2Uri(e.target.value)}
              />
            </Form.Group>
            <Form.Group className="mb-3">
              <Form.Label>DHIS2 Username</Form.Label>
              <Form.Control
                type="text"
                placeholder="Enter DHIS2 username (optional)"
                value={dhis2Username}
                onChange={(e) => setDhis2Username(e.target.value)}
              />
            </Form.Group>
            <Form.Group className="mb-3">
              <Form.Label>DHIS2 Password</Form.Label>
              <Form.Control
                type="password"
                placeholder="Enter DHIS2 password (optional)"
                value={dhis2Password}
