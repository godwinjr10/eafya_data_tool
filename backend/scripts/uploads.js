...
class CSVUploadService {
   ...
   */
  async createTableFromCSV(tableName, csvData) {
    if (!csvData || csvData.length === 0) {
      throw new Error('No data to create table from');
    }
    ...
    const createTableSQL = `
      CREATE TABLE IF NOT EXISTS reporting."${tableName}" (
  ...
  async insertData(tableName, data) {
    ...
    for (let i = 0; i < data.length; i += batchSize) {
      ...
      // Flatten data for this batch
      const values = batch.flatMap(row => 
        columns.map(col => row[col] || null)
      );

      const insertSQL = `
        INSERT INTO reporting."${tableName}" (${cleanColumns.join(', ')})
        VALUES ${placeholders}
      ...
      try {
        ...
        console.error(`Error inserting batch ${Math.floor(i / batchSize) + 1} into ${tableName}:`, error);
        throw error;
      }
    }
  ...
  async uploadCSVFile(fileName) {
    ...
    try {
      ...
      console.log(`✅ Successfully uploaded ${fileName} to reporting.${tableName}`);
      return true;
  ...
  async uploadSpecificFile(fileName) {
    ...
    if (!csvFiles.includes(fileName)) {
      throw new Error(`File ${fileName} not found in uploads directory`);
    }
  ...
  printSummary() {
    ...
    if (this.results.length > 0) {
      ...
      this.results.forEach(result => {
        console.log(`  📁 ${result.file} → ${result.table} (${result.inserted}/${result.rows} rows)`);
      });
    }
    ...
    if (this.errors.length > 0) {
      ...
      this.errors.forEach(error => {
        console.log(`  📁 ${error.file} → ${error.table}: ${error.error}`);
      });
    }

    console.log(`\n📈 Total files processed: ${this.results.length + this.errors.length}`);
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  if (specificFile) {
    ...
    uploadService.uploadSpecificFile(specificFile)
      .then(() => {
    ...
    uploadService.uploadAllCSVFiles()
      .then(() => {
        process.exit(0);
      })
      .catch(error => {
