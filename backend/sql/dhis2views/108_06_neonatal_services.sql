...
-- Deaths 0-7 days
    SUM(CASE WHEN EXTRACT(DAY FROM (newborn_date - admission_date)) BETWEEN 0 AND 7 AND time_of_death IS NOT NULL THEN 1 ELSE 0 END) AS "Deaths 0-7 days Total",
    SUM(CASE WHEN EXTRACT(DAY FROM (newborn_date - admission_date)) BETWEEN 0 AND 7 AND time_of_death IS NOT NULL AND baby_weight < 2.5 THEN 1 ELSE 0 END) AS "Deaths 0-7 days <2.5kg",
-- Deaths 8-28 days
    SUM(CASE WHEN EXTRACT(DAY FROM (newborn_date - admission_date)) BETWEEN 8 AND 28 AND time_of_death IS NOT NULL THEN 1 ELSE 0 END) AS "Deaths 8-28 days Total",
    SUM(CASE WHEN EXTRACT(DAY FROM (newborn_date - admission_date)) BETWEEN 8 AND 28 AND time_of_death IS NOT NULL AND baby_weight < 2.5 THEN 1 ELSE 0 END) AS "Deaths 8-28 days <2.5kg"
