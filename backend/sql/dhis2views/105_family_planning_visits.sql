...
WITH src AS (
  SELECT 
    ...
    CASE
      WHEN UPPER(TRIM(family_planning_method)) = 'COC'              THEN 'FPO1'
      WHEN UPPER(TRIM(family_planning_method)) = 'POP'              THEN 'FP02'
      WHEN UPPER(TRIM(family_planning_method)) = 'ECP'              THEN 'FP03'
      WHEN UPPER(TRIM(family_planning_method)) = 'DMPA'             THEN 'FP04'
      WHEN UPPER(TRIM(family_planning_method)) = '3 YRS IMPLANT'    THEN 'FP06'
      WHEN UPPER(TRIM(family_planning_method)) = '5 YRS IMPLANT'    THEN 'FP07'
      WHEN UPPER(TRIM(family_planning_method)) IN ('IUP','IUD','IUCD') THEN 'FP08'
      ELSE NULL
    END AS hmis_code,
    treatment_stage,
    CASE 
      WHEN birth_date IS NULL OR birth_date::TEXT = '00:00.0' THEN 0
      ELSE DATE_PART('year', AGE(administered_on::date, birth_date::date))
    END AS age_years
...
FROM src
GROUP BY report_month, family_planning_method, hmis_code
ORDER BY report_month, family_planning_method DESC;