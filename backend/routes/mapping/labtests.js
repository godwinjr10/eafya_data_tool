...
router.get("/", async (req, res) => {
  try {
    ...
    const countQuery = `SELECT COUNT(*)::int AS total FROM reporting.hmis_labtests ${whereSql}`;
    const dataQuery = `
      SELECT 
        section_id, 
        section_name, 
...
router.delete("/", async (req, res) => {
  try {
    const { id } = req.body;
    const sql = `DELETE FROM reporting.hmis_eafya_labtests_mapping WHERE id = $1`;
  ...
  try {
    ...
    const query = `
      SELECT 
        ...
        labtest_id, 
        labtest_name
      FROM reporting.hmis_eafya_labtests_mapping
      WHERE hmis_code = $1
    `;
...
router.post("/", async (req, res) => {
  try {
    const { hmis_code, hmis_name, labtests } = req.body;
    ...
    const columns = `section_id, hmis_code, hmis_name, labtest_id, labtest_name`;
    ...
    for (let i = 0; i < labtests.length; i++) {
      ...
      params.push(
        ...
        hmis_name || '',
        labtest.labtest_id,
        labtest.labtest_name
      );
    }

    const insertQuery = `
      INSERT INTO reporting.hmis_eafya_labtests_mapping (${columns})
