...
router.get('/conditions/:conditionId/mappings', async (req, res) => {
    try {
        ...
        console.error('Database error in /conditions/:conditionId/mappings:', error);
        res.status(500).json({ error: error.message });
    }
});
...
router.post('/conditions/:conditionId/mappings', async (req, res) => {
    try {
        ...
        if (!eafya_id || !eafya_name) {
            ...
            return res.status(400).json({ error: 'eafya_id and eafya_name are required' });
        }
        ...
        if (dimRows.length === 0) {
            ...
            return res.status(404).json({ error: 'Condition not found in dimension table' });
        }
        ...
        `;
        const { rows: duplicateCheck } = await pool.query(duplicateQuery, [conditionId, eafya_id]);
