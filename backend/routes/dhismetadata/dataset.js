...
// Note: This file is a script for fetching/saving datasets, not an Express route
...
const DHIS2_URL = `${process.env.DHIS2_BASE_URL}/dataSets`;
...
// (Removed express routes; see routes/mapping/hmis.js for mapping endpoints)

// SQL to create the table if it doesn't exist
const createTableQuery = `
CREATE TABLE IF NOT EXISTS reporting.dhis2_datasets (
    ...
    try {
        while (url) {
            ...
            if (response.data?.dataSets) {
                allData = [...allData, ...response.data.dataSets];
                console.log(`Fetched ${response.data.dataSets.length} records.`);
...
const saveToDatabase = async (data) => {
    ...
    try {
        ...
        const insertQuery = `
      INSERT INTO reporting.dhis2_datasets (code, name)
      VALUES ($1, $2)
...
const main = async () => {
    ...
    const data = await fetchDataSets(initialUrl);
