...
function buildFilters(query) {
	...
	if (lab_test_id) {
		where += ` AND lab_test_id = $${idx}`;
		params.push(lab_test_id);
		idx++;
	}
	if (parent_id) {
	...
	if (from_date) {
		where += ` AND ${date_field === 'visit_date' ? 'visit_date' : 'lab_test_date'} >= $${idx}`;
...
router.get('/', async (req, res) => {
	try {
		...
		console.error('Error fetching labtests data:', error);
		res.status(500).json({ message: 'Failed to fetch labtests data' });
	}
});
...
router.get('/export', async (req, res) => {
	try {
		...
		console.error('Error exporting labtests data:', error);
		res.status(500).json({ message: 'Failed to export labtests data' });
	}
});
